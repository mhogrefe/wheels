...
/**
 * This code is ported from a C implementation by Bob Jenkins.
 *
 * ISAAC (indirection, shift, accumulate, add, and count) is a PRNG that is cryptographically secure but also fast.
 */
public class IsaacPRNG {
    /**
     * A fixed seed, generated by {@link Random}, used by {@link IsaacPRNG#example()}.
     */
    ...
    /**
     * The log of the size of {@code result} and {@code state}.
     */
    private static final int SIZE_BITS = 8;

    /**
     * The size of {@code result} and {@code state}.
     */
    public static final int SIZE = 1 << SIZE_BITS;

    /**
     * Used for pseudorandom lookup.
     */
    private static final int MASK = (SIZE - 1) << 2;

    /**
     * A counter through the results in {@code result}.
     */
    private int count;

    /**
     * The random output of this PRNG.
     */
    private @NotNull int[] result;

    /**
     * The internal state.
     */
    private @NotNull int[] state;
    ...
    /**
     * A counter which guarantees that the cycle is at least 2<sup>40</sup>.
     */
    private int c;

    /**
     * Constructs an {@code IsaacPRNG} using {@link Random}, which uses the system time. This constructor is the
     * easiest to use, but is not secure.
     *
     * <ul>
    ...
    /**
     * Constructs an {@code IsaacPRNG} using a seed in the form of an array.
     *
     * <ul>
    ...
    public IsaacPRNG(@NotNull List<Integer> seed) {
        if (seed.size() != SIZE) {
            throw new IllegalArgumentException("The length of seed must be IsaacPRNG.SIZE. seed.size(): " +
                    seed.size() + ", IsaacPRNG.SIZE: " + SIZE);
        }
        state = new int[SIZE];
        result = new int[SIZE];
    ...
    /**
     * Resets {@code this} using a seed in the form of a list.
     *
     * <ul>
    ...
    /**
     * Generates a specific PRNG. The PRNG is not secure but may be used when deterministic results are needed (for
     * example, in unit tests).
     *
     * <ul>
     *  <li>The result is a PRNG whose seed is {@link IsaacPRNG#EXAMPLE_SEED}.</li>
     * </ul>
     *
     * @return a PRNG
     */
    ...
    /**
     * Creates a copy of {@code this} with exactly the same state.
     *
     * <ul>
     *  <li>{@code this} may be any {@code IsaacPRNG}.</li>
    ...
    public @NotNull IsaacPRNG copy() {
        IsaacPRNG copy = new IsaacPRNG(IterableUtils.toList(IterableUtils.replicate(SIZE, 0)));
        copy.a = a;
        copy.b = b;
        copy.c = c;
        copy.count = count;
    ...
    /**
     * Returns an 64-bit hash of {@code this}.
     *
     * <ul>
     *  <li>{@code this} may be any {@code IsaacPRNG}.</li>
     *  <li>The result probably may be any {@code long}.</li>
     * </ul>
     *
     * @return {@code this}'s id
     */
    public long getId() {
    ...
    /**
     * Populate {@code result} with 256 more values.
     *
     * <li>{@code this} may be any {@code IsaacPRNG}.</li>
     */
    ...
    private void initialize() {
        ...
        // The second pass makes all of seed affect all of state
        for (int i = 0; i < SIZE; i += 8) {
            a += state[i];
            b += state[i + 1];
            c += state[i + 2];
    ...
    /**
     * Ensures that {@code this} is valid. Must return without exceptions for any {@code IsaacPRNG} used outside this
     * class.
     */
    public void validate() {
        assertEquals(this, result.length, SIZE);
        assertEquals(this, state.length, SIZE);
    }
}
